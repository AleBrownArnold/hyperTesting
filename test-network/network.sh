...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-uniMadrid.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-uniBogota.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/uniMadrid/msp organizations/fabric-ca/uniMadrid/tls-cert.pem organizations/fabric-ca/uniMadrid/ca-cert.pem organizations/fabric-ca/uniMadrid/IssuerPublicKey organizations/fabric-ca/uniMadrid/IssuerRevocationPublicKey organizations/fabric-ca/uniMadrid/fabric-ca-server.db'
    docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/uniBogota/msp organizations/fabric-ca/uniBogota/tls-cert.pem organizations/fabric-ca/uniBogota/ca-cert.pem organizations/fabric-ca/uniBogota/IssuerPublicKey organizations/fabric-ca/uniBogota/IssuerRevocationPublicKey organizations/fabric-ca/uniBogota/fabric-ca-server.db'
